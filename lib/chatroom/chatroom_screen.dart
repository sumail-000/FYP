...
class _ChatRoomScreenState extends State<ChatRoomScreen>
  ...
  // New variables for message rate limiting
  int _messagesSentInLastMinute = 0;
  ...
  Future<void> _sendMessage() async {
    ...
    if (text.isEmpty || _isSending || _isRateLimited) return;

    // Check if user has sent too many messages
    if (_messagesSentInLastMinute >= 3) {
      _startRateLimitCountdown();
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('You cannot send more than 3 messages per minute.'),
    ...
    try {
      ...
      // Increment message count and check for rate limiting
      _messagesSentInLastMinute++;
      _scheduleMessageCountReset();
  ...
  // Schedule a reset of the message count after 1 minute
  void _scheduleMessageCountReset() {
    Future.delayed(Duration(minutes: 1), () {
      if (mounted) {
        setState(() {
          _messagesSentInLastMinute =
              _messagesSentInLastMinute > 0 ? _messagesSentInLastMinute - 1 : 0;
        });
      }
    });
  }
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    // Replace icon button with proper send button
                    ElevatedButton(
                      onPressed:
                          (_isSending || _isRateLimited) ? null : _sendMessage,
                      style: ElevatedButton.styleFrom(
                        backgroundColor:
                            _isRateLimited ? Colors.grey : orangeColor,
            ...
            child: Container(
              ...
              child: StreamBuilder<List<Map<String, dynamic>>>(
                ...
                builder: (context, snapshot) {
                  ...
                  if (snapshot.hasError) {
                    return Center(child: Text('Error loading online users'));
                  }
                  ...
                  return ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final isCurrentUser =
                          user['userId'] == _authService.currentUser?.uid;
                        ...
                        child: ListTile(
                          ...
                          title: Text(
                            ...
                            style: TextStyle(
                              fontWeight: FontWeight.bold,
                              color:
                                  isCurrentUser ? orangeColor : Colors.black87,
                            ),
                          ),
                          subtitle: Row(
                            children: [
          ...
          children: [
            ...
            Container(
              ...
              child: Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Icon(Icons.info_outline, size: 14, color: blueColor),
            ...
            children: [
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(
                    color:
                        isMe ? orangeColor.withOpacity(0.2) : Colors.grey[300]!,
                    width: 1,
                  ),
                ),
                child: Column(
                                ...
                                if (!isOnline && data?['lastActive'] != null) {
                                  final lastActive =
                                      (data!['lastActive'] as Timestamp)
                                          .toDate();
                                  lastSeenText =
                                      'Last seen ${_formatTimestamp(lastActive)}';
                                }
                              }
              ...
              FutureBuilder<Map<String, bool>>(
                ...
                builder: (context, snapshot) {
                  ...
                  if (snapshot.hasData && snapshot.data != null) {
                    ...
                    hasPendingRequest =
                        snapshot.data!['hasPendingRequest'] == true;
                  }
