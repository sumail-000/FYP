...
class UploadService {
  ...
  final CollectionReference _courseCodesCollection = 
      FirebaseFirestore.instance.collection('course_codes');
  ...
  final CollectionReference _courseMappingsCollection =
      FirebaseFirestore.instance.collection('course_mappings');
  ...
  Future<List<String>> getDepartments({bool forceRefresh = false}) async {
    try {
      ...
      print('Fetching departments from Firestore with forceRefresh=$forceRefresh'); // Debug print
      
      // Use Source.server to bypass cache when forceRefresh is true
      final source = forceRefresh ? Source.server : Source.serverAndCache;
      final snapshot = await _departmentsCollection.get(GetOptions(source: source));
      ...
      if (snapshot.docs.isNotEmpty) {
        ...
        print('No departments found, using defaults: $_defaultDepartments'); // Debug print
  ...
  bool _hasSimilarDepartment(List<String> existingDepartments, String newDepartment) {
    ...
    print('Checking similarity for: "$newDepartment" (normalized: "$newDeptNormalized", code: "$newDeptCode")'); // Debug print
    ...
    for (var existing in existingDepartments) {
      ...
      print('Comparing with: "$existing" (normalized: "$existingNormalized", code: "$existingDeptCode")'); // Debug print
      ...
      if (existingNormalized == newDeptNormalized) {
        print('Normalized forms match, considering as duplicate'); // Debug print
        return true;
      }
      ...
      if (similarity >= _similarityThreshold) {
        print('Similarity threshold exceeded, considering as duplicate'); // Debug print
        return true;
      }
    }
  ...
  Future<List<String>> getCourseCodes({bool forceRefresh = false}) async {
    try {
      ...
      print('Fetching course codes from Firestore with forceRefresh=$forceRefresh'); // Debug print
      
      // Use Source.server to bypass cache when forceRefresh is true
      final source = forceRefresh ? Source.server : Source.serverAndCache;
      final snapshot = await _courseCodesCollection.get(GetOptions(source: source));
      
      if (snapshot.docs.isNotEmpty) {
        // Convert Firestore documents to list of course codes
        final courseCodes = snapshot.docs
        ...
        print('No course codes found, using defaults: $_defaultCourseCodes'); // Debug print
  ...
  Future<List<String>> getCourses({bool forceRefresh = false}) async {
    try {
      ...
      print('Fetching courses from Firestore with forceRefresh=$forceRefresh'); // Debug print
      ...
      if (snapshot.docs.isNotEmpty) {
        ...
        print('No courses found, using defaults: $_defaultCourses'); // Debug print
  ...
  Future<String?> getCourseCodeForDepartmentAndCourse(String department, String course) async {
    try {
      if (department.isEmpty || course.isEmpty) return null;
      
      // Query the course mappings collection
      final snapshot = await _courseMappingsCollection
  ...
  Future<bool> saveCourseMapping(String department, String course, String courseCode) async {
    try {
  ...
  Future<bool> uploadFiles(
    ...
    final List<UploadTask> uploadTasks = uploads.map((uploadData) {
      final filePath = uploadData['filePath'] as String;
      final fileName = uploadData['displayName'] as String? ?? path.basename(filePath);
    ...
    final successfulUploads = uploadTasks.where((task) => 
        task.status == UploadStatus.completed && task.uploadedUrl != null
    ).length;
  ...
  Future<void> uploadFile(String filePath, {
    ...
    try {
      ...
      final metadata = {
        ...
        'uploadedAt': FieldValue.serverTimestamp(), // Use server timestamp instead of client time
      };
      ...
      await _documentsCollection.add({
        ...
        'resourceType': 'document', // Use generic type instead of response.resourceType
        'createdAt': FieldValue.serverTimestamp(), // Use server timestamp
        'bytes': File(filePath).lengthSync(), // Get file size from file instead of response.bytes
      });
  ...
  Future<bool> deleteDocument(String publicId, String documentId) async {
    try {
      ...
      // Step 2: Delete from Cloudinary (if available)
      // Note: Cloudinary deletion requires server-side code with API secret
