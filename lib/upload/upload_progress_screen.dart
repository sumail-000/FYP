...
class UploadProgressScreen extends StatefulWidget {
  ...
  const UploadProgressScreen({Key? key, required this.uploadTasks})
    : super(key: key);
...
class _UploadProgressScreenState extends State<UploadProgressScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final bool allFinished = widget.uploadTasks.every(
      (task) =>
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          actions: [
            ...
            TextButton(
              onPressed:
                  allFinished ? () => Navigator.of(context).pop(true) : null,
              child: Text(
                'Done',
                style: TextStyle(
                  color:
  ...
  Widget _buildUploadTaskItem(
    ...
    // Get backend progress percentage if applicable
    final backendProgressPercent =
        task.backendUploadStarted ? (task.backendProgress * 100).toInt() : 0;
    ...
    return Container(
      ...
      child: Column(
        children: [
            ...
            child: Row(
              children: [
                ...
                Container(
                  ...
                  child: Center(child: _getFileTypeIcon(extension)),
                ),
          ...
          // Add backend upload progress if applicable
          if (task.backendUploadStarted)
            Padding(
              ...
              child: Column(
                ...
                children: [
                  Row(
                    children: [
                      Icon(Icons.cloud_upload, size: 16, color: blueColor),
                      SizedBox(width: 8),
                      Text(
                        'Backend Upload:',
                        style: TextStyle(
                  ...
                  SizedBox(height: 4),
                  LinearProgressIndicator(
                    value: task.backendProgress,
                    backgroundColor: Colors.grey[300],
                    valueColor: AlwaysStoppedAnimation<Color>(
                      task.backendUploadCompleted ? Colors.green : blueColor,
                    ),
                  ),
                  if (task.backendUploadCompleted &&
                      task.backendDocumentId != null)
                    Padding(
                      padding: EdgeInsets.only(top: 4),
                      child: Text(
                        'Document ID: ${task.backendDocumentId}',
    ...
    return showDialog<bool>(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text(
            'If you leave now, all pending uploads will be cancelled. Are you sure you want to exit?',
          ),
          actions: [
            TextButton(
...
class UploadTask {
  ...
  String? backendStatusUrl;
  bool backendUploadStarted = false;
  bool backendUploadCompleted = false;
  ...
  // Add reference to backend service
      ...
      if (_fileSize > 0) {
        ...
        if (elapsedSeconds > 0) {
          ...
          final estimatedBytesPerSecond =
              (_fileSize * progress) / elapsedSeconds;
  ...
  // Perform actual upload to Cloudinary
  Future<void> _performRealUpload(
    CloudinaryService cloudinaryService,
    Function stateCallback,
  ) async {
    ...
    try {
      ...
      if (_fileSize > 100 * 1024 * 1024) {
        // > 100MB
        throw Exception(
          'File size exceeds the maximum limit (100MB). Please reduce the file size and try again.',
        );
      }
      ...
      try {
        // Start backend upload in parallel
        _uploadToBackend();
  ...
  // Upload document to backend server
  Future<void> _uploadToBackend() async {
    // Check if backend is configured
    if (!_backendService.isConfigured) {
      print('Backend not configured, skipping backend upload');
      return;
    }

    // Avoid duplicate uploads
    if (backendUploadStarted) return;
    backendUploadStarted = true;

    try {
      print('Starting backend upload for: $fileName');

      // Copy metadata for backend
      final backendMetadata = Map<String, dynamic>.from(metadata);

      // Add extra info for backend processing
      backendMetadata['fileName'] = fileName;

      // Get user data
      final userData = await _authService.getUserData();

      // Add user information to metadata
      backendMetadata['uploaderId'] = _authService.currentUser?.uid;
      backendMetadata['uploaderName'] =
          userData?.name ??
          _authService.currentUser?.displayName ??
          'Anonymous';
      backendMetadata['uploaderEmail'] =
          userData?.email ?? _authService.currentUser?.email;
      backendMetadata['university'] = userData?.university;

      // Upload to backend
      final result = await _backendService.uploadDocument(
        filePath,
        backendMetadata,
        progressCallback: (backendProgress) {
      ...
      if (result['success'] == true) {
        backendDocumentId = result['document_id'];
        backendStatusUrl = result['status_url'];
        backendUploadCompleted = true;
        print('Backend upload successful: $backendDocumentId');
      } else {
        print('Backend upload failed: ${result['message']}');
      }
    } catch (e) {
  ...
  Future<void> _saveToFirestore(CloudinaryResponse response) async {
    try {
      ...
      final documentsCollection = FirebaseFirestore.instance.collection(
        'documents',
      );
      ...
      final documentData = {
        ...
        // Add backend document info if available
        'backendDocumentId': backendDocumentId,
        'backendStatusUrl': backendStatusUrl,
        'backendProcessed': backendUploadCompleted,
      };
      ...
      print(
        'Document metadata saved to Firestore for: ${metadata['displayName'] ?? fileName}',
      );
    } catch (e) {
  ...
  Future<void> _saveMapping() async {
    try {
      ...
      final courseMappingsCollection = FirebaseFirestore.instance.collection(
        'course_mappings',
      );

      // Check if mapping already exists
      final snapshot =
          await courseMappingsCollection
