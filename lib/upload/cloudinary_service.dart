...
class CloudinaryService {
  ...
  Future<CloudinaryResponse> uploadFile(String filePath, {
    ...
    try {
      if (!isConfigured) {
        throw Exception('Cloudinary is not configured. Please set your cloud name and upload preset.');
      }
      ...
      if (kIsWeb) {
        ...
        if (_isFileTooLarge(file)) {
          ...
          final onProgressAdapter = progressCallback != null 
              ? (int count, int total) {
  ...
  Future<CloudinaryResponse> _uploadLargeFile(
    ...
    try {
      ...
      if (response.statusCode != 200) {
        final message = response.data is Map ? response.data['error']?.toString() : 'Upload failed';
        throw DioException(
      ...
      return CloudinaryResponse(
        ...
        originalFilename: responseData['original_filename'] ?? file.path.split('/').last,
  ...
  Future<List<CloudinaryResponse>> uploadFiles(List<String> filePaths, {
    ...
    for (int i = 0; i < filePaths.length; i++) {
      ...
      try {
        final response = await uploadFile(filePath, folder: folder, metadata: metadata);
    ...
    if (failedUploads.isNotEmpty) {
      throw Exception('Failed to upload ${failedUploads.length} files: ${failedUploads.join(', ')}');
    }
